  ...
  return (
    ...
    >
      <svg width="100%" height="100%" viewBox={`0 0 ${width} ${height}`} className="overflow-visible bg-[#050505]">
        ...
        {gridLines.map((p, i) => (
           <g key={i}><line x1="0" y1={getY(p)} x2={width} y2={getY(p)} stroke="#151515" strokeWidth="1"/><text x={width-45} y={getY(p)+3} fill="#cccccc" fontSize="9" fontFamily="monospace">{p.toFixed(2)}</text></g>
        ))}
        ...
        <g clipPath="url(#chart-area)">
          ...
          {visibleItems.map(item => {
             ...
             const c = item.candle; const isUp = c.close >= c.open; const color = isUp ? '#ffffff' : '#a855f7';
        ...
        <line x1="0" y1={currentPriceY} x2={width} y2={currentPriceY} stroke={isMarketClosed ? '#444' : ((data.length > 1 ? data[data.length-1].close >= data[data.length-2].close : true) ? '#ffffff' : '#a855f7')} strokeDasharray="4 4" opacity="0.8"/>
        <rect x={width-Y_AXIS_WIDTH} y={currentPriceY-9} width={Y_AXIS_WIDTH} height={18} fill={isMarketClosed ? '#333' : ((data.length > 1 ? data[data.length-1].close >= data[data.length-2].close : true) ? '#ffffff' : '#a855f7')} rx="2"/>
        <text x={width-Y_AXIS_WIDTH+4} y={currentPriceY+4} fill="#000000" fontSize="10" fontWeight="bold" fontFamily="monospace">{currentPrice.toFixed(2)}</text>
        {showIndicators && (
