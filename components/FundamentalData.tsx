    ...
    useEffect(() => {
        const fetchCalendarData = async () => {
            try {
                ...
                const targetUrl = `/api/calendar?from=${from}&to=${to}&lang=id&auth_key=[AUTH_KEY]:VINZX_OMEGA_2026`;
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[9px] text-zinc-400 font-mono mt-1 bg-black/30 px-1.5 py-0.5 rounded w-fit border border-white/5">
                                                {isToday 
                                                    ? `HARI INI • JAM ${eventDate.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })} ${!isPast ? `(${(() => {
                                                        const diff = eventDate.getTime() - currentTime;
                                                        const h = Math.floor(diff / (1000 * 60 * 60));
                                                        const m = Math.floor((diff % (1000 * 60 * 60)) / (1000 * 60));
                                                        return h > 0 ? `Dalam ${h}j ${m}m` : `Dalam ${m}m`;
                                                    })()})` : ''}`
                                                    : eventDate.toLocaleDateString('id-ID', { weekday: 'short', month: 'short', day: 'numeric' })
                                                }
                                            </span>
                                        </div>
                                    </div>
